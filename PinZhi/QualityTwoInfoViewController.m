//
//  QualityTwoInfoViewController.m
//  PinZhi
//
...
@interface QualityTwoInfoViewController ()
...
@implementation QualityTwoInfoViewController
...
-(void)useAFNetWorking
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:WANCHENG_URL,_useInfo]];
    NSLog(@"url -- %@",url);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![quality.Reback isEqual:[NSNull null]])
    {
        cell.informationTF.text = quality.Reback;
    }
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    else
    {
        ...
        if ([string isEqualToString:@"1"]) {
            ...
            //            UIAlertView *alter=[[UIAlertView alloc]initWithTitle:@"提示：" message:@"提交成功" delegate:self cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
            //            [alter show];
        }else{
             footBtn.userInteractionEnabled=YES;
            [self.view.window showHUDWithText:@"提交失败" Type:ShowPhotoNo Enabled:YES];
            //            UIAlertView *alter=[[UIAlertView alloc]initWithTitle:@"提示：" message:@"提交失败" delegate:self cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil];
            //            [alter show];
        }
