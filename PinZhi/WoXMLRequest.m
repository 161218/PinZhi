...
-(void)SoapJiud{
    ...
    NSString *soapMessage = [NSString stringWithFormat:
                             ...
                             "</soap:Body>\n"
                             "</soap:Envelope>",hotelid];
    NSLog(@"调用Picker的字符串是:%@",soapMessage);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RELOADVOEWNOTIFICATIINN" object:_infoArray];
    ...
    {
        if ([_infoArray[i] isEqualToString:hotelid])
        {
            NSString *hotelid = _infoArray[i+1];
            NSLog(@"hotelid -- %@",hotelid);
        }
    }
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    if ([elementName isEqualToString:_matchingElement]) {
        ...
        NSLog(@"hotelid---%lu",(unsigned long)_infoArray.count);
        NSLog(@"hotelid---%@",_infoArray);
}
@end
