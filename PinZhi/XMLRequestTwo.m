...
-(void)SoapJiud{
    ...
    NSString *departmentid = [userDefault objectForKey:@"employeeid"];
    
    // 设置我们之后解析XML时用的关键字，与响应报文中Body标签之间的getMobileCodeInfoResult标签对应
    _matchingElement = @"GetsinglehotelResult";
    ...
    NSString *soapMessage = [NSString stringWithFormat:
                             ...
                             //封装好主要数据  其他部分都是死的
                             "<Getsinglehotel xmlns=\"http://outpre.4008266333.net/\">"
                             "<name>%@</name>"
                             "</Getsinglehotel>"
                             
                             "</soap:Body>\n"
                             "</soap:Envelope>",departmentid];
    NSLog(@"调用webserivce的字符串是2:%@",soapMessage);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RELOADVOEWNOTIFICATIIN" object:_infoArray];
