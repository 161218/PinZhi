...
#import "Friend.h"
#import "QualityTwoInfoViewController.h"
...
@interface QualityTwoViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation QualityTwoViewController
...
//    self.tableview.backgroundView = view;
    ...
    if (_qualityArray.count==0) {
        self.navigationItem.title = [NSString stringWithFormat:@"完    成 (0条)"];
    }
...
//    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    ...
    __autoreleasing WancViewCell *cell = [[WancViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
...
//    cell.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg-page"]];
    ...
    BOOL isOpen = [self moreTimer:quality.EndDate];
    if (isOpen == NO)
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"finish-01.png"]];
        imageView.frame = CGRectMake([[UIScreen mainScreen] applicationFrame].size.width - 100, 20, 15, 15);
        [cell addSubview:imageView];
    }
    else
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"edit.png"]];
        imageView.frame = CGRectMake([[UIScreen mainScreen] applicationFrame].size.width - 100, 20, 15, 15);
        [cell addSubview:imageView];
    }
...
//    }
    ...
    QualityTwoInfoViewController *viewController = [[QualityTwoInfoViewController alloc] init];
    viewController.Time=isOpen;
    //未完成 已完成 任务数组
...
-(void)useAFNetWorking
{
    ...
    if (_timeString == nil)
    {
        ...
        url = [NSURL URLWithString:[NSString stringWithFormat:DAIBAN_URL,@"1",employeeid,hotelid]];
    }
    else
    {
        NSArray *array = [_timeString componentsSeparatedByString:@"&"];
        url = [NSURL URLWithString:[NSString stringWithFormat:DAIBAN_URL_NEW,employeeid,hotelid,@"1",array[0],array[1]]];
        NSLog(@"2222 -- %@",url);
...
- (void)requestFinished:(ASIHTTPRequest *)request{
    ...
    if ([jsonObject isKindOfClass:[NSArray class]]) {
        ...
        if (_qualityArray.count==0) {
            ...
            self.navigationItem.title = [NSString stringWithFormat:@"完    成 (%lu条)",(unsigned long)_qualityArray.count];
        }
